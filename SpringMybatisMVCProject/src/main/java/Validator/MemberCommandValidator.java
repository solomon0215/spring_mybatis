...
public class MemberCommandValidator implements Validator {
	...
	private String birthRegExp = "^\\d{4}-\\d{2}-\\d{2}$";
	...
	public void validate(Object target, Errors errors) {
		...
		if (memCmd.getUserEmail() == null || memCmd.getUserEmail().trim().isEmpty()) {
		...
		if(memCmd.getUserBirth() == null || memCmd.getUserBirth().trim().isEmpty() ) {
		...
		if (!memCmd.getUserPw().isEmpty()) {
			if (!memCmd.isPwEqualToConfirmPwCon()) {
